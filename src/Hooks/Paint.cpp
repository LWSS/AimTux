            ...
            for (const DrawRequest &value : Draw::drawRequests) {
                switch (value.type) {
                    ...
                    case DRAW_RECT_FILLED:
                        ...
                        break;
                    case DRAW_CIRCLE:
                        Draw::Circle(Vector2D(value.x0, value.y0), value.circleSegments, value.circleRadius,
                                     Color::FromImColor(value.color));
                        break;
                    case DRAW_CIRCLE_FILLED:
                        Draw::FilledCircle(Vector2D(value.x0, value.y0), value.circleSegments, value.circleRadius,
                                           Color::FromImColor(value.color));
                        break;
                    case DRAW_CIRCLE_3D:
                        Draw::Circle3D(value.pos, value.circleSegments, value.circleRadius,
                                       Color::FromImColor(value.color));
                        break;
                    case DRAW_TEXT:
