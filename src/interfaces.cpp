...
void Interfaces::FindInterfaces() {
    ...
    entityList = GetInterface<IClientEntityList>(XORSTR("./csgo/bin/linux64/client_panorama_client.so"),
                                                 XORSTR("VClientEntityList"));
    ...
    inputSystem = GetInterface<IInputSystem>(XORSTR("./bin/linux64/inputsystem_client.so"),
                                             XORSTR("InputSystemVersion"));
    inputInternal = GetInterface<IInputInternal>(XORSTR("./bin/linux64/vgui2_client.so"), XORSTR("VGUI_InputInternal"));
    material = GetInterface<IMaterialSystem>(XORSTR("./bin/linux64/materialsystem_client.so"),
                                             XORSTR("VMaterialSystem"));
    ...
    gameEvents = GetInterface<IGameEventManager2>(XORSTR("./bin/linux64/engine_client.so"),
                                                  XORSTR("GAMEEVENTSMANAGER002"), true);
    physics = GetInterface<IPhysicsSurfaceProps>(XORSTR("./bin/linux64/vphysics_client.so"),
                                                 XORSTR("VPhysicsSurfaceProps"));
    prediction = GetInterface<IPrediction>(XORSTR("./csgo/bin/linux64/client_panorama_client.so"),
                                           XORSTR("VClientPrediction001"), true);
    gameMovement = GetInterface<IGameMovement>(XORSTR("./csgo/bin/linux64/client_panorama_client.so"),
                                               XORSTR("GameMovement"));
    ...
    panoramaEngine = GetInterface<IPanoramaUIEngine>(XORSTR("./bin/linux64/panorama_client.so"),
                                                     XORSTR("PanoramaUIEngine001"), true);
    fileSystem = GetInterface<IFileSystem>(XORSTR("./bin/linux64/filesystem_stdio_client.so"), XORSTR("VFileSystem"));
    gameTypes = GetInterface<IGameTypes>(XORSTR("./csgo/bin/linux64/matchmaking_client.so"),
                                         XORSTR("VENGINE_GAMETYPES_VERSION002"), true);
}
