...
void SDL2::SwapWindow(SDL_Window *window) {
    ...
    if (!context) {
        ...
        */
        ...
        io.Fonts->AddFontFromMemoryCompressedTTF(SegoeUI_compressed_data, SegoeUI_compressed_size, 18.0f, &config,
                                                 SegoeUI_ranges);

        // Enable MergeMode and add additional fonts
        config.MergeMode = true;
        io.Fonts->AddFontFromMemoryCompressedBase85TTF(KaiGenGothicCNRegular_compressed_data_base85, 14.0f, &config,
                                                       KaiGenGothicCNRegular_ranges);
        io.Fonts->Build();
    }
