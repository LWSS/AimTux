        ...
        if (strstr(buf, XORSTR("player/land")) != nullptr) {
            {
                ...
                playerFootsteps[sounds[i].m_nSoundSource].emplace_back(*sounds[i].m_pOrigin, (Util::GetEpochTime() +
                                                                                              Settings::ESP::Sounds::time));
            } // RAII mutex lock
...
static void DrawBox(ImColor color, int x, int y, int w, int h, C_BaseEntity *entity) {
    if (Settings::ESP::Boxes::type == BoxType::FRAME_2D) {
        ...
        Draw::AddRectFilled(x, y, x + w, y + h,
                            ImColor(color.Value.x, color.Value.y, color.Value.z, 21 * (1.0f / 255.0f)));
...
static void DrawSkeleton(C_BasePlayer *player, C_BasePlayer *localplayer) {
    ...
    for (int i = 0; i < pStudioModel->numbones; i++) {
        ...
        if (debugOverlay->ScreenPosition(
                Vector(pBoneToWorldOut[i][0][3], pBoneToWorldOut[i][1][3], pBoneToWorldOut[i][2][3]), vBonePos1))
            continue;

        Vector vBonePos2;
        if (debugOverlay->ScreenPosition(
                Vector(pBoneToWorldOut[pBone->parent][0][3], pBoneToWorldOut[pBone->parent][1][3],
                       pBoneToWorldOut[pBone->parent][2][3]), vBonePos2))
            continue;

        Draw::AddLine(vBonePos1.x, vBonePos1.y, vBonePos2.x, vBonePos2.y,
                      Entity::IsTeamMate(player, localplayer) ? Settings::ESP::Skeleton::allyColor.Color()
...
static void DrawHeaddot(C_BasePlayer *player) {
    ...
    Draw::AddCircleFilled(head2D.x, head2D.y, Settings::ESP::HeadDot::size, ESP::GetESPPlayerColor(player, bIsVisible),
                          10);
}
...
static void DrawSounds(C_BasePlayer *player, ImColor playerColor) {
    ...
    if (lock.owns_lock()) {
        ...
        for (auto it = playerFootsteps[player->GetIndex()].begin();
            ...
            Draw::AddCircle3D(it->position, circleRadius, ImColor(drawColor.r, drawColor.g, drawColor.b, drawColor.a),
                              (int) points);
        }
...
static void DrawPlayerText(C_BasePlayer *player, C_BasePlayer *localplayer, int x, int y, int w, int h) {
    ...
    if (Settings::ESP::Info::name || Settings::ESP::Info::clan) {
        ...
        Draw::AddText(x + (w / 2) - (nameSize.x / 2), (y - textSize.y - nameOffset), displayString.c_str(),
    ...
    if (Settings::ESP::Info::steamId) {
        ...
        Draw::AddText((x + (w / 2) - (rankSize.x / 2)), (y - (textSize.y * lineNum) - nameOffset), playerInfo.guid,
                      Entity::IsTeamMate(player, localplayer) ? Settings::ESP::allyInfoColor.Color()
                                                              : Settings::ESP::enemyInfoColor.Color());
        lineNum++;
    }
    ...
    if (Settings::ESP::Info::rank) {
        ...
        if (rank >= 0 && rank < 19) {
            ...
            Draw::AddText((x + (w / 2) - (rankSize.x / 2)), (y - (textSize.y * lineNum) - nameOffset), ESP::ranks[rank],
                          Entity::IsTeamMate(player, localplayer) ? Settings::ESP::allyInfoColor.Color()
                                                                  : Settings::ESP::enemyInfoColor.Color());
        }
    }
    ...
    if (Settings::ESP::Info::reloading) {
        ...
        for (int i = 0; i <= layers->Count(); i++) {
            if (player->GetSequenceActivity(layers->operator[](i).m_nSequence) ==
                (int) CCSGOAnimStatePoses::ACT_CSGO_RELOAD && layers->operator[](i).m_flWeight != 0.f)
    ...
    if (Settings::Debug::AnimLayers::draw) {
        CUtlVector<AnimationLayer> *layers = player->GetAnimOverlay();
        for (int i = 0; i <= layers->Count(); i++) {
            stringsToShow.push_back(
                    Util::GetActivityName(player->GetSequenceActivity(layers->operator[](i).m_nSequence)));
        }
    }


    for (unsigned int i = 0; i < stringsToShow.size(); i++) {
        Draw::AddText(x + w + boxSpacing, (y + (i * (textSize.y + 2))), stringsToShow[i].c_str(),
                      Entity::IsTeamMate(player, localplayer) ? Settings::ESP::allyInfoColor.Color()
                                                              : Settings::ESP::enemyInfoColor.Color());
    }
}
...
static void DrawPlantedBomb(C_PlantedC4 *bomb, C_BasePlayer *localplayer) {
    ...
    ImColor color = bomb->GetBombDefuser() != -1 || bomb->IsBombDefused() ? Settings::ESP::bombDefusingColor.Color()
    ...
    if (bomb->IsBombDefused() || !bomb->IsBombTicking() || bombTimer <= 0.f) {
        ...
        displayText << XORSTR("Bomb: ") << std::fixed << std::showpoint << std::setprecision(1) << bombTimer
                    << XORSTR(", damage: ") << (int) damage;
    }
        ...
        if (client->m_ClassID == EClassIds::CCSPlayer) {
            ...
            if (player == localplayer) {
                ...
                if (!Entity::IsTeamMate(player, localplayer)) {
                   ...
                   (strstr(client->m_pNetworkName, XORSTR("Weapon")) || client->m_ClassID == EClassIds::CDEagle ||
...
static void DrawFOVCrosshair() {
    ...
    radius = std::min(radius,
                      (((180.f / OverrideView::currentFOV) * Paint::engineWidth) / 2)); // prevents a big radius (CTD).

    if (Settings::ESP::FOVCrosshair::filled)
        Draw::AddCircleFilled(Paint::engineWidth / 2, Paint::engineHeight / 2, radius,
                              Settings::ESP::FOVCrosshair::color.Color(), std::max(12, (int) radius * 2));
    else
    ...
    for (int i = 1; i < entityList->GetHighestEntityIndex(); i++) {
        ...
        if (client->m_ClassID == EClassIds::CCSPlayer &&
            (Settings::ESP::Filters::enemies || Settings::ESP::Filters::allies ||
             (Settings::ESP::Filters::localplayer && Settings::ThirdPerson::enabled))) {
        ...
        if ((client->m_ClassID != EClassIds::CBaseWeaponWorldModel &&
             (strstr(client->m_pNetworkName, XORSTR("Weapon")) || client->m_ClassID == EClassIds::CDEagle ||
              client->m_ClassID == EClassIds::CAK47 || client->m_ClassID == EClassIds::CBreachCharge ||
              client->m_ClassID == EClassIds::CBumpMine)) && client->m_ClassID != EClassIds::CPhysPropWeaponUpgrade &&
            ...
            if (Settings::ESP::DangerZone::safe && client->m_ClassID == EClassIds::CBRC4Target)
                ...
                DrawLootCrate(entity, localplayer);
                     ...
                     (client->m_ClassID == EClassIds::CMelee || client->m_ClassID == EClassIds::CKnife)) {
...
void ESP::CreateMove(CUserCmd *cmd) {
    ...
    if (Settings::ESP::enabled && Settings::ESP::Sounds::enabled &&
        (Settings::ESP::Filters::allies || Settings::ESP::Filters::enemies || Settings::ESP::Filters::localplayer)) {
        CheckActiveSounds();
    }
}
