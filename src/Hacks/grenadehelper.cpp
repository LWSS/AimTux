...
static void DrawAimHelp(GrenadeInfo *info) {
    ...
    // Draw Help line
    Draw::AddLine(Paint::engineWidth / 2, Paint::engineHeight / 2, posVec.x, posVec.y,
                  Settings::GrenadeHelper::aimLine.Color());
}
...
static void AimAssist(CUserCmd *cmd) {
    ...
    for (auto act = Settings::GrenadeHelper::grenadeInfos.begin();
         act != Settings::GrenadeHelper::grenadeInfos.end(); act++) {
...
void GrenadeHelper::Paint() {
    ...
    for (auto grenadeInfo = Settings::GrenadeHelper::grenadeInfos.begin();
         grenadeInfo != Settings::GrenadeHelper::grenadeInfos.end(); grenadeInfo++) {
