...
void NoSmoke::FrameStageNotify(ClientFrameStage_t stage) {
    ...
    for (auto material_name : smoke_materials) {
        IMaterial *mat = material->FindMaterial(material_name, TEXTURE_GROUP_CLIENT_EFFECTS);
        mat->SetMaterialVarFlag(MATERIAL_VAR_WIREFRAME, !Settings::ESP::enabled || !Settings::NoSmoke::enabled ? false :
                                                        Settings::NoSmoke::type == SmokeType::WIREFRAME);
        mat->SetMaterialVarFlag(MATERIAL_VAR_NO_DRAW, !Settings::ESP::enabled || !Settings::NoSmoke::enabled ? false :
                                                      Settings::NoSmoke::type == SmokeType::NONE);
    }
}
