...
struct mstudiobone_t
{
	...
	int surfacepropLookup; // this index must be cached by the loader, not saved in the
						   // file
...
struct studiohdr_t
{
	...
	int RemapSeqBone(int iSequence, int iLocalBone) const; // maps local sequence bone to
														   // global bone
	int RemapAnimBone(int iAnim, int iLocalBone) const; // maps local animations bone to
														// global bone
	...
	int iRelativeAnim(int baseseq, int relanim) const; // maps seq local anim reference
													   // to global anim index
	int iRelativeSeq(int baseseq, int relseq) const; // maps seq local seq reference to
													 // global seq index

	// private:
	mutable int activitylistversion; // initialization flag - have the sequences been
									 // indexed?
	...
	// this is stored here as a global store to ensure the staged loading matches the
	// rendering
	unsigned char rootLOD;

	// set in the mdl data to specify that lod configuration should only allow first
	// numAllowRootLODs to be set as root LOD:
	//	numAllowedRootLODs = 0	means no restriction, any lod can be set as root lod.
	//	numAllowedRootLODs = N	means that lod0 - lod(N-1) can be set as root lod, but
	//not lodN or lower.
	...
	// FIXME: Remove when we up the model version. Move all fields of studiohdr2_t into
	// studiohdr_t.
	int studiohdr2index;

	// NOTE: No room to add stuff? Up the .mdl file format version
	// [and move all fields in studiohdr2_t into studiohdr_t and kill studiohdr2_t],
	// or add your stuff to studiohdr2_t. See NumSrcBoneTransforms/SrcBoneTransform for
	// the pattern to use.
	int unused2[1];
...
public:
	...
	{
		typedef studiohdr_t* (*oGetModelMaterials)(
			void*, const model_t*, int, IMaterial**);
	...
	model_t* FindOrLoadModel(const char* name)
	{
		...
		return getvfunc<oFindOrLoadModel>(this, 42)(this, name);
	}
};
