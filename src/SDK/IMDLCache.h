...
public:
	...
	virtual virtualmodel_t*
	GetVirtualModelFast(const studiohdr_t* pStudioHdr, MDLHandle_t handle) = 0;
	...
	// returns a pointer to a counter that is incremented every time the cache has been
	// out of the locked state (EVIL)
	...
	virtual vcollide_t*
	GetVCollideEx(MDLHandle_t handle, bool synchronousLoad = true) = 0;
	...
	// Hammer uses this. If a model has an error loading in GetStudioHdr, then it is
	// flagged as an error model and any further attempts to load it will just get the
	// error model. That is, until you call this function. Then it will load the correct
	// model.
