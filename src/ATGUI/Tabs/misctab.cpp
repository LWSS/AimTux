...
void Misc::RenderTab() {
    ...
    ImGui::Columns(2, nullptr, true);
    {
        ImGui::BeginChild(XORSTR("Child1"), ImVec2(0, 0), true);
        {
            ...
            {
                ImGui::PushItemWidth(
                        Settings::AutoStrafe::type == AutostrafeType::AS_RAGE ? ImGui::CalcItemWidth() : -1);
            ...
            ImGui::NextColumn();
            {
                ...
                if (ImGui::BeginPopup(XORSTR("addinfo_throw"))) {
                    ...
                    if (ImGui::Button(XORSTR("Add")) && engine->IsInGame() &&
                        ...
                        if (strlen(inputName) > 0) {
                            GrenadeInfo n = GrenadeInfo((GrenadeType) gType, localPlayer->GetEyePosition(),
                                                        *localPlayer->GetVAngles(), (ThrowType) tType, inputName);
                    ...
                    ImGui::ListBox("", &throwMessageCurrent, lambda, nullptr,
    ...
    ImGui::NextColumn();
    {
        ImGui::BeginChild(XORSTR("Child2"), ImVec2(0, 0), true);
        {
            ...
            {
                ImGui::PushItemWidth(-1);
                if (ImGui::Combo(XORSTR("##ANIMATIONTYPE"), (int *) &Settings::ClanTagChanger::type, animationTypes,
                                 IM_ARRAYSIZE(animationTypes)))
                    ClanTagChanger::UpdateClanTagCallback();
