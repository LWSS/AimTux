...
void PlayerList::RenderWindow() {
    if (Settings::UI::Windows::Playerlist::reload) {
        ImGui::SetNextWindowPos(
                ImVec2(Settings::UI::Windows::Playerlist::posX, Settings::UI::Windows::Playerlist::posY),
                ImGuiSetCond_Always);
        ImGui::SetNextWindowSize(
                ImVec2(Settings::UI::Windows::Playerlist::sizeX, Settings::UI::Windows::Playerlist::sizeY),
                ImGuiSetCond_Always);
    ...
    if (ImGui::Begin(XORSTR("Player list"), &PlayerList::showWindow,
        ...
        if (engine->IsInGame() && *csPlayerResource) {
            ...
            for (int team = (int) TeamID::TEAM_UNASSIGNED; team <= (int) TeamID::TEAM_COUNTER_TERRORIST; team++) {
                ...
                for (auto it : players[(TeamID) team]) {
                    ...
                    ImGui::Text("%d/%d/%d", (*csPlayerResource)->GetKills(it), (*csPlayerResource)->GetAssists(it),
                                (*csPlayerResource)->GetDeaths(it));
                    ImGui::NextColumn();
        ...
        if (currentPlayer != -1) {
            ...
            ImGui::Columns(3);
            {
                bool isFriendly = std::find(Aimbot::friends.begin(), Aimbot::friends.end(), entityInformation.xuid) !=
                                  Aimbot::friends.end();
                if (ImGui::Checkbox(XORSTR("Friend"), &isFriendly)) {
                    if (isFriendly)
                        Aimbot::friends.push_back(entityInformation.xuid);
                    else
                        Aimbot::friends.erase(
