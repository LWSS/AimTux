...
char buildID[] = "TYOe1ewiEuCEzYTw0w9CdURXOqsu453H"; // Line Set by build script
// ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
bool preload = false;
...
void MainThread()
{
	...
	if( preload )
	{
		while( !isShuttingDown )
		{
			...
			{
				cvar->ConsoleColorPrintf(ColorRGBA(225, 0, 0), XORSTR("DEBUGGER DETECTED! EXITING Skinchanger\n"));
