...
bool
Hooker::HookRecvProp(const char *className, const char *propertyName, std::unique_ptr<RecvPropHook> &recvPropHook) {
...
void Hooker::FindPlayerAnimOverlayOffset() {
    ...
    Offsets::playerAnimOverlayOffset = *(unsigned int *) (
            PatternFinder::FindPatternInModule(XORSTR("client_panorama_client.so"),
                                               (unsigned char *) XORSTR(
...
void Hooker::FindSequenceActivity() {
    ...
    uintptr_t funcAddr = PatternFinder::FindPatternInModule(XORSTR("client_panorama_client.so"),
